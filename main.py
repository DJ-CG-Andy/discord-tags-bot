...
@bot.tree.command(name="checkin-starter", description="設定簽到用的 GIF/貼圖（管理員）")
@app_commands.describe(
    media_url="簽到用的 GIF 或圖片網址"
)
async def checkin_starter(interaction: discord.Interaction, media_url: str):
    """設定簽到用的 GIF/貼圖（管理員）"""
    if not interaction.user.guild_permissions.administrator:
        await interaction.response.send_message("❌ 你沒有權限使用此命令（需要管理員權限）", ephemeral=True)
        return

    await db.set_checkin_config(str(interaction.guild_id), media_url, updated_by=str(interaction.user.id))

    embed = discord.Embed(
        title="✅ 簽到設定成功",
        description="簽到用媒體已設定完成",
        color=discord.Color.green()
    )
    embed.add_field(name="媒體網址", value=media_url, inline=False)
...
async def checkin(interaction: discord.Interaction):
    ...
    # 檢查是否已簽到
    if await db.has_checked_in_today(guild_id, user_id):
        embed = discord.Embed(
            title="⚠️ 重複簽到",
            description=f"{interaction.user.mention} 你今天已經簽到過了，明天再來吧！",
    ...
    # 取得簽到配置
    config = await db.get_checkin_config(guild_id)
    if not config:
        await interaction.response.send_message("❌ 管理員尚未設定簽到用的媒體，請聯絡管理員使用 `/checkin-starter` 設定", ephemeral=True)
        return

    # 記錄簽到
    await db.add_checkin_record(guild_id, user_id)

    embed = discord.Embed(
        title="✅ 簽到成功",
        description=f"{interaction.user.mention} 今日簽到完成！",
        color=discord.Color.green()
    )
