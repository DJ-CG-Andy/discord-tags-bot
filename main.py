...
async def on_message(message: discord.Message):
    ...
    print(
        f"📝 訊息內容: {message.content[:100] if message.content else '(無文字)'}",
        flush=True,
    )
    ...
    if gif_request:
        ...
        if message.attachments:
            ...
            for i, attachment in enumerate(message.attachments):
                print(
                    f"🔍 附件 {i}: {attachment.filename}, 類型: {attachment.content_type}",
                    flush=True,
                )
        ...
        if gif_url:
            ...
            await checkin_manager.set_config(
                guild_id, channel_id, gif_request["checkin_time"], gif_url
            )
                ...
                if gif_url:
                    ...
                    if message.attachments:
                        for attachment in message.attachments:
                            if (
                                ...
                                print(
                                    f"🔍 從附件獲取到 URL: {sent_gif_url}", flush=True
                                )
                                # 從 URL 中提取 ID
                                import re
                    ...
                    if message.content:
                        ...
                        if re.match(r"^\d{16,}$", content_stripped):
                            ...
                            if not sent_gif_id:
                                ...
                                print(
                                    f"🔍 從訊息內容提取到純 GIF ID: {sent_gif_id}",
                                    flush=True,
                                )
                        else:
                            # 檢查是否有 URL
                    ...
                    if gif_id and sent_gif_id:
                        if sent_gif_id == gif_id:
                            ...
                            print(
                                f"🔍 GIF ID 不匹配: 設定={gif_id}, 發送={sent_gif_id}",
                                flush=True,
                            )
                    ...
                    if not is_match and sent_gif_url:
                        ...
                        print(
                            f"🔍 發送的 URL - Path: {sent_parsed.path}, Netloc: {sent_parsed.netloc}",
                            flush=True,
                        )
                        print(
                            f"🔍 設定的 URL - Path: {config_parsed.path}, Netloc: {config_parsed.netloc}",
                            flush=True,
                        )

                        url_match = (
                            sent_parsed.path == config_parsed.path
                            and sent_parsed.netloc == config_parsed.netloc
                        )
                    ...
                    if is_match:
                        ...
                        # 執行簽到
                        success, total, streak = await checkin_manager.checkin(
                            user_id, guild_id
                        )
                        ...
                        if success:
                            ...
                            embed.add_field(
                                name="總簽到次數", value=f"📊 {total} 次", inline=True
                            )
                            embed.add_field(
                                name="連續簽到", value=f"🔥 {streak} 天", inline=True
                            )
                            embed.add_field(
                                name="簽到時間",
    ...
    if add_request:
        ...
        if message.attachments:
            for attachment in message.attachments:
                if attachment.content_type and "image" in attachment.content_type:
                    ...
                    print(
                        f"✅ 從附件獲取: {trigger_type}, ID: {trigger_id}, URL: {trigger_url}",
                        flush=True,
                    )
                    break
        ...
        if not trigger_type and message.stickers:
            ...
            print(
                f"✅ 從貼圖獲取: {trigger_type}, ID: {trigger_id}, URL: {trigger_url}",
                flush=True,
            )
        ...
        if not trigger_type and message.content:
            ...
            if emojis:
                ...
                print(
                    f"✅ 從表情符號獲取: {trigger_type}, ID: {trigger_id}, URL: {trigger_url}",
                    flush=True,
                )
        ...
        if trigger_type and trigger_id:
            ...
            if success:
                ...
                for t in triggers:
                    print(
                        f"🔍 觸發器: ID={t['trigger_id'][-8:]}, type={t['trigger_type']}",
                        flush=True,
                    )
    ...
    if guild_id and channel_id:
        ...
        print(
            f"🔍 配置頻道 ID: {config.get('channel_id') if config else None}",
            flush=True,
        )
        ...
        if (
            config
            and str(config.get("channel_id")) == channel_id
            ...
            if message.attachments:
                for attachment in message.attachments:
                    if attachment.content_type and "image" in attachment.content_type:
                        ...
                        trigger_type = "gif"
                        print(
                            f"🔍 從附件提取觸發器: {trigger_type}, ID: {trigger_id}",
                            flush=True,
                        )
                        break
            ...
            if not trigger_id and message.stickers:
                ...
                trigger_type = "sticker"
                print(
                    f"🔍 從貼圖提取觸發器: {trigger_type}, ID: {trigger_id}", flush=True
                )
            ...
            if not trigger_id and message.content:
                ...
                if emojis:
                    ...
                    trigger_type = "emoji"
                    print(
                        f"🔍 從表情符號提取觸發器: {trigger_type}, ID: {trigger_id}",
                        flush=True,
                    )
            ...
            try:
                ...
                for trigger in triggers:
                    print(
                        f"🔍 檢查觸發器: ID={trigger['trigger_id'][-8:]} (目標: {trigger_id[-8:]})",
                        flush=True,
                    )
                    if trigger["trigger_id"] == trigger_id:
                        ...
                        print(
                            f"🔍 準備發送回覆: type={reply_trigger_type}, url={reply_trigger_url}",
                            flush=True,
                        )
                        ...
                        if reply_trigger_type == "gif" and reply_trigger_url:
                            ...
                            async def download_url_to_temp_file(url: str) -> str:
                                ...
                                ext = (
                                    os.path.splitext(path)[1] if "." in path else ".gif"
                                )
                            ...
                            temp_path = await download_url_to_temp_file(
                                reply_trigger_url
                            )
                            try:
        ...
        for tag in tags:
            ...
            if compare_emoji(tag.emoji, payload.emoji):
                ...
                if success:
                    ...
                    embed.add_field(
                        name="標籤", value=f"{emoji_display} {tag.name}", inline=True
                    )
                    if tag.description:
                        embed.add_field(name="說明", value=tag.description, inline=True)
                    embed.add_field(
                        name="操作者", value=f"<@{payload.user_id}>", inline=True
                    )
        ...
        for tag in tags:
            ...
            if compare_emoji(tag.emoji, payload.emoji):
                ...
                if has_other_reactions:
                    print(
                        f"訊息 {message.id} 上還有其他人在使用 emoji {tag.emoji}，不刪除標籤"
                    )
                    return
...
async def checkin_scheduler():
    ...
    while not bot.is_closed():
        try:
            ...
            if now.minute == 0:
                print(
                    f"🕐 定時任務運行中 - 當前時間: {current_time} ({current_date})",
                    flush=True,
                )
            ...
            for guild in bot.guilds:
                ...
                if config:
                    print(
                        f"🔍 配置存在 - 簽到時間: {config.get('checkin_time')}, 頻道 ID: {config.get('channel_id')}",
                        flush=True,
                    )
                else:
                ...
                if config and config["checkin_time"] == current_time:
                    ...
                    if channel:
                        ...
                        # 發送總簽到次數排行榜
                        leaderboard_total = await checkin_manager.get_leaderboard(
                            guild_id, limit=10, by_streak=False
                        )

                        if leaderboard_total:
                            embed = discord.Embed(
                                title="📊 總簽到次數排行榜", color=discord.Color.gold()
                            )
                            ...
                            embed.set_footer(
                                text=f"更新時間: {now.strftime('%Y-%m-%d %H:%M:%S')}"
                            )
                        ...
                        # 發送連續簽到排行榜
                        leaderboard_streak = await checkin_manager.get_leaderboard(
                            guild_id, limit=10, by_streak=True
                        )

                        if leaderboard_streak:
                            embed = discord.Embed(
                                title="🔥 連續簽到排行榜", color=discord.Color.orange()
                            )
                    ...
                    if config:
                        print(
                            f"⏰ 時間不匹配: 設置時間={config['checkin_time']}, 當前時間={current_time}",
                            flush=True,
                        )
...
async def on_ready():
    ...
    if os.path.exists(DB_PATH):
        print(
            f"🔍 資料庫初始化後文件大小: {os.path.getsize(DB_PATH)} bytes", flush=True
        )
    ...
    await bot.change_presence(
        activity=discord.Activity(
            type=discord.ActivityType.watching, name=f"{config.get('prefix', '!')}menu"
        )
    )
...
class MainMenuView(View):
    ...
    async def view_tags(
        ...
        try:
            ...
            if not tags:
                ...
                embed.add_field(
                    name="💡 提示",
                    value="使用 `!menu` 點擊「🏷️ 新增標籤」來創建新標籤",
                    inline=False,
                )
            ...
            for i, tag in enumerate(tags):
                print(
                    f"🔍 標籤 {i}: name={tag.name}, emoji={tag.emoji}, emoji_type={type(tag.emoji)}"
                )
                # 使用 display_emoji 來顯示 emoji 或圖片連結
                emoji_display = display_emoji(tag.emoji)
    ...
    async def advanced_features(
        ...
        embed.add_field(
            name="📥 導入歷史", value="導入頻道的歷史訊息並添加標籤", inline=False
        )
        embed.add_field(
            name="📊 統計數據", value="查看系統統計和標籤使用情況", inline=False
        )
...
class BackToMenuView(View):
    ...
    async def back_to_menu(
        ...
        is_checkin_channel = (
            checkin_config and checkin_config["channel_id"] == channel_id
        )
        ...
        is_reply_channel = (
            reply_config
            and reply_config.get("channel_id") == channel_id
            and reply_config.get("enabled")
        )
        print(f"🔍 is_reply_channel: {is_reply_channel}", flush=True)

        # 根據情況選擇 View 和 embed
        if is_checkin_channel and is_reply_channel:
            view = MainMenuViewWithCheckinAndReply(
        ...
        if is_reply_channel:
            embed.add_field(
                name="🎭 刷版區設定", value="設置刷版區回覆功能", inline=False
            )
...
class MainMenuViewWithCheckin(View):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.primary,
        emoji="🏷️",
        custom_id="add_tag_with_checkin",
    )
    async def add_tag(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="🔍",
        custom_id="search_tag_with_checkin",
    )
    async def search_tag(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="📋",
        custom_id="view_tags_with_checkin",
    )
    async def view_tags(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    async def checkin_settings(
        ...
        embed.add_field(
            name="⏰ 調整時間", value="調整每日簽到的時間界限", inline=False
        )
        embed.add_field(
            name="🖼️ 更換 GIF", value="更換簽到成功時顯示的 GIF", inline=False
        )
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.success,
        emoji="📥",
        custom_id="advanced_features_with_checkin",
    )
    async def advanced_features(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
...
class MainMenuViewWithReply(View):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="🔍",
        custom_id="search_tag_with_reply",
    )
    async def search_tag(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="📋",
        custom_id="view_tags_with_reply",
    )
    async def view_tags(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.success,
        emoji="🎭",
        custom_id="reply_settings_with_reply",
    )
    async def reply_settings(
        ...
        embed = discord.Embed(
            title="🎭 刷版區設定", description=description, color=discord.Color.purple()
        )
        embed.add_field(name="⚙️ 設置頻道", value="設置當前頻道為刷版區", inline=False)
        embed.add_field(
            name="🖼️ 新增回覆",
            value="新增想要 bot 回覆的 GIF/貼圖/表情符號",
            inline=False,
        )
        embed.add_field(name="📊 顯示排名", value="查看觸發回覆排行榜", inline=False)
        embed.add_field(
            name="🗑️ 刪除回覆",
            value="刪除想要 bot 回覆的 GIF/貼圖/表情符號",
            inline=False,
        )
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.success,
        emoji="📥",
        custom_id="advanced_features_with_reply",
    )
    async def advanced_features(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
...
class MainMenuViewWithCheckinAndReply(View):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.primary,
        emoji="🏷️",
        custom_id="add_tag_with_checkin_reply",
    )
    async def add_tag(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="🔍",
        custom_id="search_tag_with_checkin_reply",
    )
    async def search_tag(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="📋",
        custom_id="view_tags_with_checkin_reply",
    )
    async def view_tags(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.success,
        emoji="✨",
        custom_id="checkin_settings_with_reply",
    )
    async def checkin_settings(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.success,
        emoji="🎭",
        custom_id="reply_settings_with_checkin_reply",
    )
    async def reply_settings(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.success,
        emoji="📥",
        custom_id="advanced_features_with_checkin_reply",
    )
    async def advanced_features(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
...
class AddTagModal(Modal, title="新增標籤"):
    ...
    description = TextInput(
        label="說明/備註",
        placeholder="例如：重要知識點（可不填寫）",
        required=False,
        style=discord.TextStyle.paragraph,
    )
    image_url = TextInput(
        label="圖片連結",
        placeholder="可選：輸入圖片連結以顯示自定義emoji",
        required=False,
        max_length=200,
        style=discord.TextStyle.short,
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            print(
                f"   tag_name: {tag_name} (類型: {type(tag_name).__name__})", flush=True
            )
            print(
                f"   tag_emoji: {tag_emoji} (類型: {type(tag_emoji).__name__})",
                flush=True,
            )
            print(
                f"   tag_description: {tag_description} (類型: {type(tag_description).__name__})",
                flush=True,
            )
            print(
                f"   tag_image_url: {tag_image_url} (類型: {type(tag_image_url).__name__})",
                flush=True,
            )
            ...
            success = await tag_manager.create_custom_tag(
                tag_name, "custom", normalized_emoji, tag_description, tag_image_url
            )
            ...
            if success:
                ...
                if tag_image_url:
                    embed.add_field(
                        name="圖片", value=f"[查看圖片]({tag_image_url})", inline=False
                    )
                embed.add_field(
                    name="💡 提示",
                    value=f"現在只要有人在訊息上添加 `{emoji_display}` emoji，就會自動加入此標籤！",
                    inline=False,
                )
...
class SearchTagModal(Modal, title="搜索標籤"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not found_tag:
            await interaction.response.send_message(
                f"❌ 找不到標籤 `{tag_name_input}`，請使用 `!menu` -> `📋 查看標籤` 查看可用標籤"
            )
            return
        ...
        if not message_tags:
            ...
            embed.add_field(
                name="💡 提示",
                value=f"試試在訊息上添加 {emoji_display} emoji 來自動加入標籤！",
                inline=False,
            )
            await interaction.response.send_message(embed=embed)
            return
        ...
        for i, mt in enumerate(message_tags[:limit]):
            content = (
                mt.message_content[:100] + "..."
                if len(mt.message_content) > 100
                else mt.message_content
            )
...
class AdvancedFeaturesView(View):
    ...
    @discord.ui.button(
        label="📥 導入歷史", style=discord.ButtonStyle.primary, emoji="📥"
    )
    async def import_history(
        ...
        embed.add_field(
            name="💡 提示", value="請選擇頻道，然後輸入要導入的 emoji", inline=False
        )
    ...
    @discord.ui.button(
        label="📊 統計數據", style=discord.ButtonStyle.secondary, emoji="📊"
    )
    async def show_stats(
        ...
        try:
            ...
            embed = discord.Embed(title="📊 系統統計", color=discord.Color.purple())
            embed.add_field(
                name="🏷️ 總標籤數", value=stats.get("total_tags", 0), inline=True
            )
            embed.add_field(
                name="📝 已標記訊息",
                value=stats.get("total_tagged_messages", 0),
                inline=True,
            )
            embed.add_field(
                name="👥 活躍用戶", value=stats.get("active_users", 0), inline=True
            )

            if stats.get("top_tags"):
                top_tags_str = "\n".join(
                    [
                        f"{i + 1}. {display_emoji(tag['emoji'])} {tag['name']}: {tag['count']} 次"
                        for i, tag in enumerate(stats["top_tags"][:5])
                    ]
                )
...
class ImportHistoryModal(Modal, title="導入歷史訊息"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not found_tag:
            await interaction.response.send_message(
                f"❌ 找不到使用 Emoji `{emoji_input}` 的標籤，請先創建標籤！"
            )
            return
        ...
        # 開始導入歷史
        await interaction.response.send_message(
            f"📥 正在導入 `{channel.name}` 頻道的歷史訊息..."
        )

        # 使用 history_processor 導入歷史
        try:
            await history_processor.import_history_by_emoji(
                interaction, channel, found_tag
            )
        except Exception as e:
            print(f"導入歷史錯誤: {e}")
...
async def status_command(ctx: commands.Context):
    ...
    embed = discord.Embed(title="📊 Bot 詳細狀態", color=discord.Color.gold())
    ...
    embed.add_field(
        name="📝 已標記訊息", value=stats.get("total_tagged_messages", 0), inline=True
    )
    embed.add_field(
        name="✅ 已初始化", value="是" if _initialized else "否", inline=True
    )
...
async def set_checkin_channel_command(ctx: commands.Context):
    ...
    if success:
        ...
        embed.add_field(
            name="💡 提示",
            value="使用 `!menu` 查看主菜單，現在應該會顯示「✨ 簽到設定」選項",
            inline=False,
        )
        await ctx.send(embed=embed)
    else:
...
async def check_config_command(ctx: commands.Context):
    ...
    if config:
        embed = discord.Embed(title="📋 簽到配置", color=discord.Color.green())
        embed.add_field(name="✅ 已配置", value="是", inline=True)
        embed.add_field(
            name="📌 簽到頻道 ID", value=config.get("channel_id", "未知"), inline=False
        )
        embed.add_field(
            name="⏰ 簽到時間", value=config.get("checkin_time", "00:00"), inline=True
        )
        embed.add_field(
            name="🖼️ GIF 連結",
            value=config.get("gif_url", "無")[:50] + "..."
            if config.get("gif_url")
            else "無",
            inline=False,
        )
        embed.add_field(
            name="📅 創建時間", value=config.get("created_at", "未知"), inline=True
        )
        embed.add_field(
            name="🔄 更新時間", value=config.get("updated_at", "未知"), inline=True
        )
        await ctx.send(embed=embed)
    else:
        embed = discord.Embed(
        ...
        embed.add_field(
            name="💡 提示",
            value="請使用 `!set_checkin_channel` 在簽到頻道中設置",
            inline=False,
        )
        await ctx.send(embed=embed)
...
async def create_test_tag_command(
    ctx: commands.Context, name: str = "測試標籤", emoji: str = "🏷️"
):
    """創建測試標籤
    ...
    try:
        ...
        # 創建標籤
        success = await tag_manager.create_custom_tag(
            name, "custom", normalized_emoji, "測試標籤", ""
        )
...
async def confirm_rebuild_command(ctx: commands.Context):
    ...
    try:
        ...
        await db._execute_d1(
            "CREATE INDEX IF NOT EXISTS idx_message_tags_guild ON message_tags(guild_id)"
        )
        await db._execute_d1(
            "CREATE INDEX IF NOT EXISTS idx_message_tags_content ON message_tags(message_content)"
        )
...
async def check_instances_command(ctx: commands.Context):
    ...
    print(
        f"🔍 連接狀態: {bot.ws.status if hasattr(bot.ws, 'status') else 'unknown'}",
        flush=True,
    )
    ...
    embed.add_field(
        name="連接狀態",
        value=str(bot.ws.status) if hasattr(bot.ws, "status") else "unknown",
        inline=True,
    )
    embed.add_field(
        name="當前時間", value=datetime.now().strftime("%Y-%m-%d %H:%M:%S"), inline=True
    )
    embed.add_field(
        name="💡 提示",
        value="如果看到多個實例發送訊息，可能有多個 bot 實例在運行",
        inline=False,
    )
...
async def force_delete_all_tags_command(ctx: commands.Context):
    ...
    try:
        ...
        embed.add_field(
            name="⚠️ 警告", value="所有標籤和關聯的標籤記錄將被永久刪除", inline=False
        )
...
class ConfirmDeleteAllTagsView(View):
    ...
    async def cancel_delete(
        ...
        """取消刪除"""
        embed = discord.Embed(
            title="❌ 已取消", description="刪除操作已取消", color=discord.Color.grey()
        )
        await interaction.response.edit_message(embed=embed, view=None)
...
async def set_checkin_command(
    ctx: commands.Context, time: str = "00:00", gif_url: str = ""
):
    """設置簽到系統
    ...
    success = await checkin_manager.set_config(
        str(ctx.guild.id), str(ctx.channel.id), time, gif_url
    )
    ...
    if success:
        ...
        embed.add_field(
            name="注意", value="每天會在設定的時間自動發送簽到訊息", inline=False
        )
...
class ReplySettingsView(View):
    ...
    async def set_channel(
        ...
        print(
            f"🔍 用戶: {interaction.user.name} (ID: {interaction.user.id})", flush=True
        )
        print(
            f"🔵 頻道: {interaction.channel.name} (ID: {interaction.channel.id})",
            flush=True,
        )
        ...
        print(
            f"🔍 準備設置配置: guild_id={guild_id}, channel_id={channel_id}", flush=True
        )

        success = await self.reply_manager.set_config(
        ...
        if success:
            ...
            embed.add_field(
                name="功能說明",
                value="在此頻道中發送設定的 GIF/貼圖/表情符號時，bot 會隨機回覆",
                inline=False,
            )
            embed.add_field(
                name="新增回覆", value="使用「🖼️ 新增回覆」來添加觸發器", inline=False
            )
    ...
    async def add_reply(
        ...
        view = AddReplyConfirmationView(
            self.reply_manager, self.guild_id, str(interaction.channel.id)
        )
        await interaction.response.send_message(embed=embed, view=view)

    @discord.ui.button(label="📊 回覆排名", style=discord.ButtonStyle.secondary)
    async def show_reply_leaderboard(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    async def show_user_leaderboard(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        # 顯示用戶排名（用戶觸發bot回覆次數）
        ...
        if not stats:
            ...
            embed.add_field(
                name="💡 提示", value="用戶還沒有觸發過回覆！", inline=False
            )
        else:
            embed = discord.Embed(
                title="👤 用戶觸發排行榜",
    ...
    async def delete_reply(
        ...
        view = DeleteReplyConfirmationView(
            self.reply_manager, self.guild_id, str(interaction.channel.id)
        )
        await interaction.response.send_message(embed=embed, view=view)
    ...
    async def back_to_menu(
        ...
        view = BackToMenuView(
            guild_id=self.guild_id, channel_id=str(interaction.channel.id)
        )
        embed = discord.Embed(
            title="🎮 Discord 標籤系統",
            description="選擇一個操作：",
...
class AddReplyConfirmationView(View):
    ...
    async def confirm(
        ...
        embed.add_field(
            name="4. Discord 貼圖", value="直接發送 Discord 貼圖", inline=False
        )
        embed.add_field(
            name="5. 表情符號", value="直接發送自定義表情符號", inline=False
        )
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content="已取消新增回覆", embed=None, view=None
        )
...
class DeleteReplyConfirmationView(View):
    ...
    async def confirm(
        ...
        embed.add_field(name="發送方式", value="你可以使用以下任一方式：", inline=False)
        embed.add_field(
            name="1. GIF 連結", value="直接發送要刪除的 GIF 連結", inline=False
        )
        embed.add_field(name="2. GIF ID", value="只發送要刪除的 GIF ID", inline=False)
        embed.add_field(
            name="3. Discord 貼圖", value="直接發送要刪除的 Discord 貼圖", inline=False
        )
        embed.add_field(
            name="4. 表情符號", value="直接發送要刪除的自定義表情符號", inline=False
        )
        embed.add_field(name="⏰ 有效時間", value="2 分鐘", inline=False)
    ...
    async def cancel(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.edit_message(
            content="已取消刪除回覆", embed=None, view=None
        )
