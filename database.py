...
class Database:
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            await db.execute('''
                CREATE TABLE IF NOT EXISTS checkin_records (
                    ...
                    UNIQUE(guild_id, user_id, DATE(checked_at))
                )
            ''')
    ...
    async def get_checkin_config(self, guild_id: str) -> Optional[Dict]:
        """獲取伺服器簽到配置"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
    ...
    async def set_checkin_config(self, guild_id: str, media_url: str, media_type: str = 'gif', updated_by: str = None):
        """設定或更新伺服器簽到配置"""
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute(
                '''INSERT INTO checkin_config (guild_id, media_url, media_type, updated_by)
                   VALUES (?, ?, ?, ?)
                ...
                (guild_id, media_url, media_type, updated_by)
            )
            await db.commit()

    async def has_checked_in_today(self, guild_id: str, user_id: str) -> bool:
        """檢查用戶今天是否已簽到"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                "SELECT 1 FROM checkin_records WHERE guild_id = ? AND user_id = ? AND DATE(checked_at) = DATE('now')",
                (guild_id, user_id)
            ) as cursor:
                return await cursor.fetchone() is not None

    async def add_checkin_record(self, guild_id: str, user_id: str) -> bool:
        """新增簽到記錄，若已存在返回 False"""
        async with aiosqlite.connect(self.db_path) as db:
            try:
                await db.execute(
                    'INSERT INTO checkin_records (guild_id, user_id) VALUES (?, ?)',
                    (guild_id, user_id)
                )
                await db.commit()
                return True
    ...
    async def get_checkin_stats(self, guild_id: str, limit: int = 10) -> List[Dict]:
        """獲取伺服器簽到統計（今日排行）"""
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute(
                '''SELECT user_id, COUNT(*) as count FROM checkin_records
                   WHERE guild_id = ?
